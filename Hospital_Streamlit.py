...
def predict_readmission(Gender, Admission_Type, Diagnosis, Num_Lab_Procedures,
    ...
    with open("Readmitted_Model.pkl", "rb") as file:
...
if select == "Home":
    ...
    with col2:
        button = st.button("PREDICT THE READMISSION", use_container_width=True)
        ...
        if button:
            admission = predict_readmission(Gender, Admission_Type, Diagnosis, Num_Lab_Procedures,
                                            ...
                                            Num_Emergency_Visits, Num_Diagnoses, A1C_Result)
            if admission == 1:
                st.write("## Readmission is Required", unsafe_allow_html=True)
            else:
                st.write("## Readmission is Not Required", unsafe_allow_html=True)
